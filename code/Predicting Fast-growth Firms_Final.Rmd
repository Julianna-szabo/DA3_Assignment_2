...
#data <- read_csv('../Data/clean/cs_bisnode_panel.csv')
...
The colours on the left figure correspond to the threshold values' level. The lighter the color, the lower the threshold level. As a result, we can see that if our threshold is substantial (in the bottom left corner), there are very few cases that are predicted to be positive. Similarly, the true positive rate and the false positive rate are also low. Moreover, as we lower the threshold value, more and more positive cases will happen according to the ROC curve. This is good on one hand because a higher true-positive rate means higher accuracy. However, a higher false-positive rate is alarming because it indicates we are making more mistakes. If we are willing to make more mistakes when we accept more errors, we can set the threshold low and have a high true-positive rate at the expense of actually having a high rate of making mistakes. If it’s too costly to have positive errors, then we should increase the threshold, and accept the fact that our true-positive rate is going to be low.
...
Once we finished building the logit models, we also choose to look at a non-linear model in a Random Forest form. This model uses 5-fold cross-validation and is getting tested on a holdout set in the end. We decide to use a relatively low number of variables per tree grown to better compare with the logit models, while the node size is set relatively small to build larger trees. Based on this model, we can find the best tuning parameters for the Random Forest model and calculate the error measures. Using the above-mentioned loss function, we can determine the best threshold value using the formula and the best tuning parameters.

```{r, echo=FALSE, message=FALSE, warning=FALSE, results='hide', fig.show='hide', cache=TRUE}
#################################################
...
The below figure shows all expected loss values for the grid possible thresholds values from 0 to 1, for a single test. Both the expected loss value turns out to be 0.70 on a single test and for cross-validated average. 

```{r, message = F, echo = FALSE, warning=FALSE, fig.align='center', out.width = '50%'}
# Create plots - this is for Fold5

rf_lossplot <- createLossPlot(roc_obj, best_treshold, "rf_p_loss_plot")
# rf_rocplot <- createRocPlotWithOptimal(roc_obj, best_treshold, "rf_p_roc_plot")
rf_lossplot
...
Once we calculate all these parameters, we can run the holdout model to show the RMSE and other error measures. As we can see in the below table, the random forest performs better than the logit models in predicting probabilities, with a cross-validated RMSE of 0.424 and AUC of 0.628. The random forest's optimal threshold is similar to logit M3, and we use this value to make the classification. The cross-validated expected loss is 0.706 for the random forest, which is also smaller than the best logit model (0.712 for logit M3).  

\begin{table}[htbp]
\centering
\begin{tabular}{lccccc}
\hline\hline
 & N Predictors & CV RMSE & CV AUC & CV Threshold & Expected Loss \\ 
  \hline
Logit M1 & 11 & 0.431 & 0.574 & 0.171 & 0.735 \\ 
Logit M2 & 18 & 0.428 & 0.605 & 0.172 & 0.728 \\ 
Logit M3 & 35 & 0.426 & 0.622 & 0.165 & 0.712 \\ 
Logit M4 & 76 & 0.425 & 0.623 & 0.171 & 0.717 \\ 
Logit M5 & 150 & 0.426 & 0.630 & 0.141 & 0.716\\ 
Logit LASSO & 46 & 0.424 & 0.614 & 0.169 & 0.731 \\ 
RF Probability & 36 & 0.424 & \bf{0.628} & 0.165 & \bf{0.706} \\ 
\hline\hline
\end{tabular}
   \caption{\bf Summary of Model Performance Measures}
\end{table}


Overall, the best model in both probability prediction for a company will be fast-growing in the next year, and classification is Random Forest. While with this Machine Learning Algorithm, we cannot clearly state the importance, pinpointing this even with the logit models.
